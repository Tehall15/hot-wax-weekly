...
// Builds slot state from a reviews array for a given week key.
// Called on hydration and on week navigation — no reactive effect needed.
const EMPTY = () => ([
...
function buildSlotsFromReviews(reviews, wk) {
...
// Effect C + D live here. Responsible for Spotify only.
// No auth coordination, no hydration, no redirects beyond cleaning the OAuth URL.
function useSpotify(clientId, user) {
  ...
  useEffect(() => {
    ...
    fetch('https://accounts.spotify.com/api/token', {
      ...
      .catch(err => console.error('[C] token exchange failed:', err));
